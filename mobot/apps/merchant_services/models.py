...
class Product(Trackable):
    ...
    # FIXME: what are the right decimal places?
    price = MoneyField(max_digits=20, decimal_places=0, default_currency=PMOB, help_text='Price of the product in PMOB',
...
class OrderManager(models.Manager):
    ...
    def order_product(self, product: Product, customer: Customer) -> QuerySet:
        ...
        try:
            inventory_item = InventoryItem.objects.filter(order=None, product=product).first()
...
class Campaign(ValidatableMixin):
    ...
    # FIXME: What are the right decimal places?
    adjusted_price = MoneyField(max_digits=20, default=None, decimal_places=0, default_currency="PMB", blank=True,
                                null=True)

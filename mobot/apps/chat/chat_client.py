...
class Mobot:
    ...
    def self.
        ...
        log_and_send_message(self, customer: Customer, source: str, text: str):
    sent_message = Message(customer=customer, store=self.drop.store, text=text,
...
def _handler_handle_payment(self, source, receipt):
    ...
    while transaction_status == "TransactionPending":
        receipt_status = self.mobilecoin_client.check_receiver_receipt_status(settings.STORE_ADDRESS,

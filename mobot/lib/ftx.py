...
class OpenExchangeRatesWithFtxBackend(backends.OpenExchangeRatesBackend):
    ...
    def _get_ftx_usd_mob(self) -> requests.Response:
        ...
        return requests.get(f"{self.BASE_FTX_API}/markets/MOB/USD")
...
class PriceAPI:
    ...
    def convert(self, money: Money, target_currency: Currency) -> Money:
        ...
        converted = convert_money(money, target_currency)
        self.logger.info(f"Converted {money} to {converted}")
        return converted